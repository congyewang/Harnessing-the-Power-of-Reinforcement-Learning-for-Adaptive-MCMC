...
for model_name in pdb_model_name_list:
    with open("templete/search_step_size.py", "r") as f_py:
        py_template = jinja2.Template(f_py.read())

    with open("templete/run-search.sh", "r") as f_bash:
        bash_template = jinja2.Template(f_bash.read())

    py_context = {"model_name": model_name}
    with open(f"search_step_size_{model_name}.py", "w") as f_py:
        f_py.write(py_template.render(py_context))

    bash_context = {"model_name": model_name}
    with open(f"run-search-step_size_{model_name}.sh", "w") as f_bash:

...
OUTPUT_DIR = "./whole_results_discount_factor"
    ...
    for rl_algorithm, mcmc_env in product(rl_algorithm_list, mcmc_env_list):
        for random_seed in range(repeat_num):
            ...
            hyperparameter_template_path = (
                f"{template_root_dir}/config_template_discount_factor.toml"
            )
        ...
        pdb_run_path = f"{template_root_dir}/template_discount_factor_run.py"
        ...
        pdb_run_path = f"{model_result_dir}/run_discount_factor_{rl_algorithm}_{mcmc_env}.py"
        with open(pdb_run_path, "w") as file:
            file.write(pdb_run_content)

        bash_template_path = f"{template_root_dir}/template.run-discount_factor.sh"
        bash_context = {
            "model_name": model_name,
            "rl_algorithm": rl_algorithm,
            "mcmc_env": mcmc_env,

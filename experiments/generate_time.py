    ...
    for rl_algorithm, mcmc_env in product(rl_algorithm_list, mcmc_env_list):
        for random_seed in range(repeat_num):
            ...
            hyperparameter_template_path = (
                f"{template_root_dir}/config_template_time.toml"
            )
        ...
        pdb_run_path = f"{template_root_dir}/template_time_run.py"
        ...
        pdb_run_path = f"{model_result_dir}/run_time_{rl_algorithm}_{mcmc_env}.py"
        with open(pdb_run_path, "w") as file:
            file.write(pdb_run_content)

        bash_template_path = f"{template_root_dir}/template.run-time.sh"
        bash_context = {
            "model_name": model_name,
            "rl_algorithm": rl_algorithm,
            "mcmc_env": mcmc_env,

...
from pyrlmala.learning.callback import register_timing_callbacks
...
def get_cpu_time(
    ...
    timer = register_timing_callbacks(learning_instance)
...
def write_header(file_path: str) -> None:
    with open(file_path, "w") as file:
        file.write("model_name,rl_algorithm,mcmc_env,random_seed,cpu_time\n")


def write_results(random_seed: int, cpu_time: float, file_path: str) -> None:
    rl_algorithm = "{{ rl_algorithm }}"
    mcmc_env = "{{ mcmc_env }}"

    with open(file_path, "a+") as file:
        file.write(f"{model_name},{rl_algorithm},{mcmc_env},{random_seed},{cpu_time}\n")
...
def main():
    ...
    for random_number in range(repeat_num):
        try:
            cpu_time = get_cpu_time(random_number)
            if cpu_time:
                write_results(random_number, cpu_time, file_path)
            else:
                write_results(random_number, 999999.0, file_path)

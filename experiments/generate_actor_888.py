...
OUTPUT_DIR = "./whole_results_actor_888"
...
def generate_files(
    ...
    for rl_algorithm, mcmc_env in product(rl_algorithm_list, mcmc_env_list):
        ...
        actor_context = {"hidden_layers": "[8,8,8]"}
        ...
        pdb_run_path = f"{template_root_dir}/template_actor_888_run.py"
        ...
        pdb_run_path = f"{model_result_dir}/run_actor_888_{rl_algorithm}_{mcmc_env}.py"
        with open(pdb_run_path, "w") as file:
            file.write(pdb_run_content)

        bash_template_path = f"{template_root_dir}/template.run-actor_888.sh"
        bash_context = {
            "model_name": model_name,
            "rl_algorithm": rl_algorithm,
            "mcmc_env": mcmc_env,

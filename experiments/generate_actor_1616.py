...
OUTPUT_DIR = "./whole_results_actor_1616"
...
def create_model_result_dir(model_name: str) -> str:
    ...
    """
    model_result_dir = f"{OUTPUT_DIR}/{model_name}"
    ...
    for rl_algorithm, mcmc_env in product(rl_algorithm_list, mcmc_env_list):
        for random_seed in range(repeat_num):
            ...
            hyperparameter_template_path = (
                f"{template_root_dir}/config_template_actor.toml"
            )
        ...
        actor_context = {"hidden_layers": "[16,16]"}
        actor_template_path = f"{template_root_dir}/actor_template.toml"
        with open(actor_template_path, "r") as file:
            actor_template = jinja2.Template(file.read())
        actor_content = actor_template.render(actor_context)
        actor_config_path = f"{model_result_dir}/config/actor.toml"
        Toolbox.create_folder(actor_config_path)
        with open(
            actor_config_path,
            "w",
        ) as file:
            file.write(actor_content)

        shutil.copy(f"{template_root_dir}/critic.toml", f"{model_result_dir}/config")

        pdb_run_path = f"{template_root_dir}/template_actor_1616_run.py"
        ...
        pdb_run_path = f"{model_result_dir}/run_actor_1616_{rl_algorithm}_{mcmc_env}.py"
        with open(pdb_run_path, "w") as file:
            file.write(pdb_run_content)

        bash_template_path = f"{template_root_dir}/template.run-actor_1616.sh"
        bash_context = {
            "model_name": model_name,
            "rl_algorithm": rl_algorithm,
            "mcmc_env": mcmc_env,
